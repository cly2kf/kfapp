 ...
 */
@SuppressWarnings("ALL") public abstract class BaseSliderView
{
	...
	protected void bindEventAndShow(final View v, final ImageView targetImageView)
	{
		...
		RequestManager p = (glide != null) ? glide : Glide.with(mContext);
		BitmapTypeRequest rq = null;
		if (mUrl != null)
		{
			rq = p.load(mUrl).asBitmap();
		}
		else if (mFile != null)
		{
			rq = p.load(mFile).asBitmap();
		}
		else if (mRes != 0)
		{
			rq = p.load(mRes).asBitmap();
		}
		else
		{
			return;
		...
		rq.into(new SimpleTarget<Bitmap>()
		{
