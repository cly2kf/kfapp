...
public class SweetSheet
{
	...
	/**
	 * @param delegate 主体部分
	 * @param SSColor  顶部拖动条颜色
	 * @param BGColor  整体背景颜色
	 */
	public void setDelegate(Delegate delegate, int SSColor, int BGColor)
	{
		mDelegate = delegate;
		mDelegate.init(mParentVG, SSColor, BGColor);
		setup();
	}
	...
	{
		mDelegate = delegate;
		mDelegate.init(mParentVG, Color.WHITE, Color.WHITE);
		setup();
	}
	...
	public void show()
	{
		...
		if (mDelegate.getStatus() == Status.SHOW || mDelegate.getStatus() == Status.SHOWING ||
			mDelegate.getStatus() == Status.DISMISSING)
		{
			return;
		}
	...
	public void dismiss()
	{
		...
		if (mDelegate.getStatus() == Status.SHOWING ||
			mDelegate.getStatus() == Status.DISMISSING)
		{
			return;
		}
