...
public class SwitchAnimationUtil
{
	...
	/**
	 * @param root       根布局
	 * @param techniques {@link Techniques}类中的各种动画效果
	 */
	public void startAnimation(View root, Techniques techniques)
	{
		ViewUtils.init(root.getContext());
		bindAnimation(root, 0, techniques);
	}

	private void bindAnimation(View view, int depth, Techniques techniques)
	{
		if (view instanceof ViewGroup)
		{
			ViewGroup group = (ViewGroup) view;
			for (int i = 0; i < group.getChildCount(); i++)
			{
				bindAnimation(group.getChildAt(i), depth + 1, techniques);
			}
		}
		else
		{
			runAnimation(view, mDelay * mOrderIndex, techniques);
			mOrderIndex++;
		}
	}

	private void runAnimation(View view, long delay, Techniques techniques)
	{
