...
public class MutiProgress extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        float nodeWidth = ((float) mWidth) / (nodesNum);
        for (int i = 0; i < nodesNum; i++) {
            Node node = new Node();
            if (i == 0)
                node.mPoint = new Point(((int) nodeWidth * i) + (int) nodeWidth / 2 - nodeRadius, mHeight / 2 - nodeRadius);
    ...
    private void DrawProgerss() {
        ...
        mPaint.setStrokeWidth(nodeRadius / 4);
        //前半截线段
//        mCanvas.drawLine(nodeRadius, mHeight/2, mWidth-nodeRadius, mHeight/2, mPaint);  //线段2端去掉nodeRadius
        mCanvas.drawLine(0, mHeight / 2, nodes.get(currNodeNO).mPoint.x + nodeRadius, nodes.get(currNodeNO).mPoint.y + nodeRadius, mPaint);  //线段2端去掉nodeRadius
        //后半截线段
        mPaint.setColor(Color.parseColor("#404040"));
        mCanvas.drawLine(nodes.get(currNodeNO).mPoint.x + nodeRadius, nodes.get(currNodeNO).mPoint.y + nodeRadius, mWidth, mHeight / 2, mPaint);  //线段2端去掉nodeRadius
    }
